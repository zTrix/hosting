...
!function(a,b){function c(){var b=a.navigator.userAgent,c=b.indexOf("MSIE "),d=b.indexOf("Trident/");if(c>0)return parseInt(b.substring(c+5,b.indexOf(".",c)),10);if(d>0){var e=b.indexOf("rv:");return parseInt(b.substring(e+3,b.indexOf(".",e)),10)}return!1}function d(a,b,d){var e=y.slice(a,b+1).join("").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;");for(c()&&(e=e.replace(/(%[^\n]*)\n/g,"$1<br/>\n"));b>a;)y[b]="",b--;y[a]="@@@@"+v.length+"@@@@",d&&(e=d(e)),v.push(e),m=w=x=null}function e(a){m=w=x=null,v=[];var b;/`/.test(a)?(a=a.replace(/~/g,"~T").replace(/(^|[^\\])(`+)([^\n]*?[^`\n])\2(?!`)/gm,function(a){return a.replace(/\$/g,"~D")}),b=function(a){return a.replace(/~([TD])/g,function(a,b){return{T:"~",D:"$"}[b]})}):b=function(a){return a},y=z(a.replace(/\r\n?/g,"\n"),/(\$\$?|\\(?:begin|end)\{[a-z]*\*?\}|\\[\\{}$]|[{}]|(?:\n\s*)+|@@@@\d+@@@@)/i);for(var c=1,e=y.length;e>c;c+=2){var f=y[c];"@"===f.charAt(0)?(y[c]="@@@@"+v.length+"@@@@",v.push(f)):m?f===w?n?x=c:d(m,c,b):f.match(/\n/)&&"$"==w?(m=w=x=null,n=0):f.match(/\n.*\n/)?(x&&(c=x,d(m,c,b)),m=w=x=null,n=0):"{"===f?n++:"}"===f&&n&&n--:"$"===f||"$$"===f?(m=c,w=f,n=0):"begin"===f.substr(1,5)&&(m=c,w="\\end"+f.substr(6),n=0)}return x&&d(m,x,b),ret=b(y.join(""))}function f(a){return a=a.replace(/@@@@(\d+)@@@@/g,function(a,b){return v[b]}),v=null,a}b.head=b.getElementsByTagName("head")[0],"getElementsByClassName"in b||(b.getElementsByClassName=function(a){function c(a,b){for(var c=[],d=new RegExp("(^| )"+b+"( |$)"),e=a.getElementsByTagName("*"),f=0,g=e.length;g>f;f++)d.test(e[f].className)&&c.push(e[f]);return c}return c(b.body,a)});var g=b.getElementsByTagName("xmp")[0]||b.getElementsByTagName("textarea")[0],h=b.getElementsByTagName("title")[0],i=b.getElementsByTagName("script"),j=b.getElementsByClassName("navbar")[0];if(!g)return void console.warn("No embedded Markdown found in this document for Strapdown.js to work on! Visit http://strapdownjs.com/ to learn more.");b.body.style.display="none";var k=b.createElement("meta");k.name="viewport",k.content="width=device-width, initial-scale=1.0, maximum-scale=1.0, minimum-scale=1.0",b.head.firstChild?b.head.insertBefore(k,b.head.firstChild):b.head.appendChild(k);for(var l="",m=0;m<i.length;m++)i[m].src.match("strapdown")&&(l=i[m].src);var o=l.substr(0,l.lastIndexOf("/")),p=g.getAttribute("theme")||"bootstrap";p=p.toLowerCase();var q=b.createElement("link");q.href=o+"/themes/"+p+".min.css",q.rel="stylesheet",b.head.appendChild(q);var q=b.createElement("link");q.href=o+"/strapdown.min.css",q.rel="stylesheet",b.head.appendChild(q);var q=b.createElement("link");q.href=o+"/themes/bootstrap-responsive.min.css",q.rel="stylesheet",b.head.appendChild(q);var r=g.textContent||g.innerText,s=b.createElement("div");s.className="container",s.id="content",b.body.replaceChild(s,g);var s=b.createElement("div");if(s.className="navbar navbar-fixed-top",!j&&h){s.innerHTML='<div class="navbar-inner"> <div class="container"> <div id="headline" class="brand"> </div> </div> </div>',b.body.insertBefore(s,b.body.firstChild);var t=h.innerHTML,u=b.getElementById("headline");u&&(u.innerHTML=t)}var v,m,w,x,y,z=3==="aba".split(/(b)/).length?function(a,b){return a.split(b)}:function(a,b){var c,d=[];if(!b.global){c=b.toString();var e="";c=c.replace(/^\/(.*)\/([im]*)$/,function(a,b,c){return e=c,b}),b=RegExp(c,e+"g")}for(var f=b.lastIndex=0;c=b.exec(a);)d.push(a.substring(f,c.index)),d.push.apply(d,c.slice(1)),f=c.index+c[0].length;return d.push(a.substring(f)),d},A=e(r),B=marked(A),C=f(B);if(b.getElementById("content").innerHTML=C,C!=B){var D=b.createElement("script");D.type="text/javascript",D.src="http://cdn.mathjax.org/mathjax/latest/MathJax.js?config=TeX-AMS_HTML";var E=function(){MathJax.Hub.Config({tex2jax:{inlineMath:[["$","$"]],displayMath:[["$$","$$"]],processEscapes:!0,balanceBraces:!0},messageStyle:"none"}),MathJax.Hub.Queue(["Typeset",MathJax.Hub])};D.onload=E,D.onreadystatechange=function(){"complete"==this.readyState&&E()},b.getElementsByTagName("head")[0].appendChild(D)}for(var F=b.getElementsByTagName("code"),m=0,G=F.length;G>m;m++){var H=F[m],I=H.className;H.className="prettyprint lang-"+I}prettyPrint();for(var J=b.getElementsByTagName("table"),m=0,G=J.length;G>m;m++){var K=J[m];K.className="table table-striped table-bordered"}b.body.style.display=""}(window,document);