...
!function(a,b){var c=new Persist.Store("strapdown_editor",{swf_path:"/persist.swf"}),d=b.getElementsByTagName("xmp")[0]||b.getElementsByTagName("textarea")[0],e=d.getAttribute("version"),f=a.location.pathname+"#"+e;c.get(f,function(a,d){var e=ace.edit("editor"),g=e.getSession(),h=!1;e.setTheme("ace/theme/monokai"),g.setMode("ace/mode/markdown"),g.setTabSize(2),g.setUseSoftTabs(!0),a&&d&&confirm("Detected unsaved document cache, do you want to load the cache?")&&g.setValue(d);var i=b.getElementById("savValue"),j=b.getElementsByTagName("form")[0];j.addEventListener("submit",function(){i.value=e.getValue(),c.set(f,g.getValue()),h=!0});var k=Date.now()-2e3;e.on("change",function(a){var b=Date.now();b-k>2e3&&(c.set(f,g.getValue()),k=b)}),b.getElementsByTagName("body")[0].addEventListener("unload",function(){h||c.set(f,g.getValue())})})}(window,document);